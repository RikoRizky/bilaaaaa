function init() {
  ...
  // Check if .scene exists, if not, just setup gift box click
  if (!svg) {
    setupGiftBox();
    return;
  }
  ...
  var skiliftDom = svg.querySelector("#skilift"),
    ...
    handHat = snowManHat ? snowManHat.querySelector("#handhat_1_") : null,
    eyesHat = snowManHat ? snowManHat.querySelector("#eyesHatMan") : null,
    elfMan = svg.querySelector("#elfman"),
    elfButtons = elfMan ? elfMan.querySelectorAll("#elfButtons circle") : [],
    elfBow = elfMan ? elfMan.querySelectorAll("#bowElf") : [],
...
function setupGiftBox() {
  function openBox() {
    ...
    gift.removeEventListener("click", openBox);
    
    // Animasi topi terbang ke kiri atas
    ...
    // Fade out gift container dengan transisi smooth
    TweenMax.to(gift, 1, {
      opacity: 0,
      delay: 0.8,
      ease: Power2.easeIn,
